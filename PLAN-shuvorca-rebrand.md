...
| D4 | Mobile / App Store CTAs and defaults | **Keep mobile; preserve the distinct "Orca Mobile" name** (exempt from the i18n transform via a `(?! Mobile)` lookahead) so the fork never presents a nonexistent "shuvorca Mobile". Button stays visible (`showMobileButton` unchanged). The inline "(upstream)" suffix was dropped in implementation — it read poorly in nav buttons; the distinct name already signals the upstream product. **⚠️ Superseded by ADR-0003 once the fork ships its own mobile app:** the fork now ships `shuvorca` mobile, so the `(?! Mobile)` exemption is removed and desktop CTAs repoint to fork store/release URLs (see `PLAN-shuvorca-mobile-rebrand.md` Phase 2). |
